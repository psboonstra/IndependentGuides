...
by commas, that *map* certain variables fin your dataset to certain aesthetics. 
...
We could alternatively the `id` variable in `growth_data` to the `color` aesthetic:
...
This is a bit strange-looking because ggplot is interpreting `months` as a number --
...
while it technically allows us to distinguish between differenb values of 
...
If desired, you can save this script as `geom_point.R`, for example, and then create
a new script before moving on to the next topic. This is not mandatory, though. 
...
squiggles up and down because the `y` values are not guaranteed to be increasing,
but it will never squiggle left or right because the `x` values are explicitly
pair to be increasing. 
...
If desired, you can save this script as `more_geoms.R`, for example, and create
a new script before moving on to the next topic. This is not mandatory, though. 
...
this plot does an exceptional job of showing the relationship between weight and height. 
...
As before, you can save your work in this script, using a name like `layering_geoms.R`, 
for example. 
