...
There are many reasons why it's helpful to be able to generate random numbers from 
a distribution. In our case, we want to generate random numbers from a distribution
to demonstrate use of the function for conducting t-tests in R. 
...
actually pre-determined sequences of numbers that are nearly indistinguishable 
from truly random numbers. One implication of this is that if you start at the 
same point in the sequence as someone else, you will obtain the same set of "random"
numbers.
...
The output is a tibble (remember from Course 1 that this is one way to store data
