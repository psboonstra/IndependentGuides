...
In the third lesson of Week 2, you were introduced to additional functions from
...
explicitly named, the arguments do not have to be in any particular order. The
`id_cols` argument specifies the variable that uniquely identifies each row,
which is the infant ID variable in this example. The `names_from` argument
...
Notice that R doesn't allow numeric variable names, so backticks are placed
around the number in each column to create valid variable names. However, these
variable names are not particularly informative of the data contained in each
column. In this case, the `names_prefix` argument is useful for adding a string
before every variable name. Returning to our example, this argument allows us to
...
"all columns except" within this argument (e.g., pivot all columns except the
infant ID variable).
...
* not every data wrangling step will be for dramatic changes or additions to data
* focus on certain columns or sort rows of data based on values of particular variables
* information in data will remain the same (re-arranging order of rows)
* arrange order of rows, even  by multiple variables (breaking ties)
* extracting only a few variables to view reduced version of data (select function)
* select based upon negations (- or !)

To practice the `arrange` and `select` functions from the dplyr package, open
another blank script within the same R project.
...
However, it's not always efficient to name every column, especially if there are
several columns in the data frame. In this case, there are helper functions that
